...
class Wallet(models.Model):
    ...
    @classmethod
    def deposit_amount(cls, amount: float) -> Tuple[float, float]:
        """
        ...
        try:
            with transaction.atomic():
                ...
                cls.real_money += abs(amount)
                if amount > cls.deposit_bonus_amount:
                    # if amount is greater than
                    # x euros we grant customers a bonus
                    bonus = Bonus(wallet=cls)
                    bonus.bonus_money += cls.casino_bonus_deposit
                    bonus.wagering()
                cls.save()
        except IntegrityError:
            print('Could not deposit')

        return cls.real_money, bonus.bonus_money
